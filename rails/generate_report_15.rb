...
def get_collection_url(collection)
  ...
  "https://era.library.ualberta.ca/communities/%{community_id}/collections/%{collection_id}" % {community_id: collection.community.id, collection_id: collection.id}
end
...
CSV.open(file_name, 'wb', write_headers: true, headers: ['Collection title', 'Collection URL']) do |csv|
  Collection.find_each do |collection|
    unless collection.member_objects.length > 5
